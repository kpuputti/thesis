...
\fixme{Add dev headaches: device categories, refreshes in dev, cache
  updates}
...
and might add up to \abbr{UI} artifacts and slowness.
...
\abbr{UI} reflows to improve the perceived performance.
...
make the \abbr{UI} components a lot more responsive.
...
users, but rather for a single conference use with some hundreds
users.
...
in the tool itself, since the inline script tags were the JavaScript
templates for the single-page application. The \texttt{type} attribute
of the tags was \texttt{text/x-handlebars-template}, which complies
with the generic extending mechanism as specified in the HTML5
specification draft \cite{HTML5draft}. Thus this was not seen as a
problem.
