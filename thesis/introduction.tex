...
\item \textbf{Animations}
  ...
  and the end. Animations can also be configured to repeat a certain
  number of times, to alternate between the begin and end values, to
  control the running and paused states, and to delay the start
  time. \cite{CSSanimations}
...
not affect the main \abbr{UI} thread apart from the communication
...
\abbr{REST} \citationneeded API as a backend data layer, and using an
...
Single-page applications might introduce a bigger initial page load,
but after the startup, the network is only used when interacting with
the backend data API. This makes the applications faster and more
responsive, and minimizes the effects that flaky networks have on the
application usage since separate page views do not require a new
document request.
...
\item \textbf{Collection}
  ...
  Collections are ordered sets of models. They can be used observe and
  manipulate models as a group. They can also be used to filter
  specific models for some purposes.

\item \textbf{Router}

  Routers provide methods for routing between pages of an application
  by observing or modifying the \abbr{URL}. URLs can be mapped to
...
\begin{itemize}
\item scale UI to different screens
\item use same code for mobile, tablets, and desktop
\item media queries
\end{itemize}
...
\begin{itemize}
\item introduce a few example frameworks/libraries for building mobile
  web apps
\end{itemize}
...
\begin{itemize}
\item combining web apps with native technologies
\item level of web/native functionality be pretty much anything
\item end result is an installable native app with web tech in it
\item introduce Phonegap as an example tool
\item app can be uploaded to an app store
\end{itemize}
