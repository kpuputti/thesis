...
\begin{itemize}
\item define what meant with HTML5 \cite{HTML5draft}
\item explain background research on common classes and ids that
  influenced the new elements
...
\item explain DOCTYPE and standards mode triggering
\end{itemize}

\subsection{Semantic Markup, Form Enhancements, and Extensibility}
...
Each element in HTML5 is in zero or more content categories that group
elements with similar characteristics \cite{HTML5draft}:
...
\fixme{Add ARIA semantics 3.2.7 \cite{HTML5draft}}

\noindent \textbf{Extensibility (2.3.2 in \cite{HTML5draft}):}
\begin{itemize}
\item class attr
\item data-*='''' attrs
\item meta tags (and name extensions)
\item rel attr (and link type extensions)
\item script tags with custom type attr
\item embed element and plugins
\item extending JavaScript using the prototyping mechanism
\item vendor-specific features
\item elements and attrs that are not understood should be kept
  neutral
\end{itemize}
