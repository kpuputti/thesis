...
the Web that relate to what we can do with HTML, \abbr{CSS} (Cascading
...
\item \textbf{Selectors}
  ...
  \abbr{CSS3} (Cascading Style Sheets level 3) defines a set of new
...
\abbr{WebGL} (Web Graphics Library) is a low-level 3D rendering API
derived from the \abbr{OpenGL® ES} 2.0 (Open Graphics Library for
...
manipulated in memory or as \abbr{Blob} (Binary Large Object)
\abbr{URLs} with the \abbr{DOM} API. These capabilities enable web
...
such as web browser, and a remote server. Because \abbr{HTTP}
...
\abbr{HTTPS} (Hypertext Transfer Protocol Secure). The API uses a
single \abbr{TCP} (Transmission Control Protocol) connection that is
...
a single \abbr{IP} (Internet Protocol) address, mechanism to overcome
...
The whole \abbr{IT} (Information Technology) industry has had a
...
cores is increasing in modern \abbr{CPUs} (Central Processing
...
site. The worker runs in a separate \abbr{OS}-level (Operating System)
...
be the \abbr{GPS} (Global Positioning System) chip on a mobile device
or a \abbr{WiFi} (Wireless Local Area Network, WLAN) network with a
